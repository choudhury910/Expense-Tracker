...
displayContainer.addEventListener('click',(e)=>{
    if(e.target.id=='dltbtn'){
        ...
        axios.post(`http://database-1.c4dse0mj8b7w.us-east-1.rds.amazonaws.com:5000/deleteExpense/${trId}`,{},{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
        .then((res)=>{
            tr.remove()
            notifyUser(res.data.message)
        })
...
downloadBtn.addEventListener('click',()=>{
    axios.get('http://54.226.239.21:5000/download',{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(response=>{
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    axios.get('http://54.226.239.21:5000/previousdownloads',{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(response=>{
