...
form.addEventListener('submit',(e)=>{
    ...
    description.value='';
    category.value='';
    axios.post('http://54.226.239.21:5000/addExpense',obj,{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(res=>{
...
function displayExpenses(limit,page=1,rows=localStorage.getItem('rows')){
    ...
    axios.get(`http://database-1.c4dse0mj8b7w.us-east-1.rds.amazonaws.com:5000/getExpenses?limit=${limit}&page=${page}&rows=${rows}`,{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
    .then(response=>{
...
displayContainer.addEventListener('click',(e)=>{
    if(e.target.id=='dltbtn'){
        ...
        //li.remove()
        axios.post(`http://database-1.c4dse0mj8b7w.us-east-1.rds.amazonaws.com:5000/deleteExpense/${liId}`,{},{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
        .then((res)=>{
            li.remove()
            notifyUser(res.data.message)
        })
...
premiumBtn.addEventListener("click", () => {
    ...
    axios.post('http://54.226.239.21:5000/premium/create/order',{amount:amount},{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})    
    .then(response=>{
...
payBtn.addEventListener('click',(e)=>{
    ...
    var options = {
        ...
        "handler": function (response){
            ...
            axios.post('http://54.226.239.21:5000/transaction/detail',{orderId:orderId,paymentId:paymentId},{headers:{'authorization':`Bearer ${localStorage.getItem('token')}`}})
            .then()
            .catch(err=>{
                console.log(err)
            })
